...
type ContractorDashboardNavigationProp = StackNavigationProp<RootStackParamList, 'ContractorDashboard'>;
...
const TaskCard = styled.TouchableOpacity<{ status: 'pending' | 'submitted' | 'approved' | 'overdue' }>`
  ...
  border-left-color: ${props => 
    ...
    props.status === 'submitted' ? colors.primary :
    props.status === 'approved' ? colors.success : colors.error
  };
  shadow-color: #000;
...
const TaskStatus = styled.View<{ status: 'pending' | 'submitted' | 'approved' | 'overdue' }>`
  background-color: ${props => 
    props.status === 'pending' ? colors.warning + '20' :
    props.status === 'submitted' ? colors.primary + '20' :
    props.status === 'approved' ? colors.success + '20' : colors.error + '20'
  };
  padding: 4px 8px;
  border-radius: 8px;
`;

const TaskStatusText = styled.Text<{ status: 'pending' | 'submitted' | 'approved' | 'overdue' }>`
  font-size: 12px;
  font-weight: 600;
  color: ${props => 
    props.status === 'pending' ? colors.warning :
...
export default function ContractorDashboard({ navigation }: Props) {
  ...
  const actions = [
    { title: 'View All Tasks', subtitle: 'See complete task list and details', icon: '📋' },
    { title: 'Update Progress', subtitle: 'Add photos and progress updates', icon: '📸' },
    { title: 'Submit Completion', subtitle: 'Mark tasks as completed', icon: '✅' },
    { title: 'Request Extension', subtitle: 'Request deadline extension', icon: '⏰' },
    { title: 'Messages', subtitle: 'Communicate with project admin', icon: '💬' },
  ];
  ...
  const handleTaskPress = (task: any) => {
    Alert.alert(
      ...
      [
        ...
        { text: 'Update Progress', onPress: () => handleActionPress('Update Progress') },
        task.status === 'pending' && { 
          text: 'Mark Complete', 
        ...
        <StatsContainer>
          {stats.map((stat, index) => (
            <StatCard key={index} style={{ width: '48%' }}>
              ...
              <StatValue style={{ 
                color: stat.label === 'Days to Deadline' && parseInt(stat.value) <= 3 ? colors.error : colors.primary 
              }}>
                {stat.value}
              </StatValue>
